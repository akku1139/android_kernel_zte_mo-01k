				 ...
				 struct inode *inode, blkcnt_t *count)
{
	...
	if (uid_gte(sbi->s_resuid, current_fsuid()) || gid_gte(sbi->s_resgid, current_fsgid()) ||
		...
		if (unlikely(sbi->total_valid_block_count > avail_user_block_count - f2fs_r_blocks_count(sbi)))
			WARN(1,"f2fs: reserved blocks will be allocated, current_fsuid = %u, current_fsgid = %u, avail_user_block_count = %u, total_valid_block_count = %u\n",
					from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()), avail_user_block_count, sbi->total_valid_block_count );
		...
		if (unlikely(sbi->total_valid_block_count > avail_user_block_count)) {
			...
			*count -= diff;
			...
			if (!*count) {
				...
				WARN(1,"f2fs: no reserved block left, current_fsuid = %u, current_fsgid = %u\n",
		...
		if (unlikely(sbi->total_valid_block_count > (avail_user_block_count - f2fs_r_blocks_count(sbi)))) {
			...
			release = *count;
			*count = 0;
			WARN(1,"f2fs: no free block left 1, current_fsuid = %u, current_fsgid = %u, avail_user_block_count = %u, total_valid_block_count = %u\n",
				from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()), avail_user_block_count, sbi->total_valid_block_count );
