				 ...
				 struct inode *inode, blkcnt_t *count)
{
	...
	if (uid_gte(sbi->s_resuid, current_fsuid()) || gid_gte(sbi->s_resgid, current_fsgid()) ||
		...
		if (unlikely(sbi->total_valid_block_count > (sbi->user_block_count - f2fs_r_blocks_count(sbi))))
			WARN(1,"no free block left, reserved blocks will be allocated, current_fsuid = %u, current_fsgid = %u, user_block_count = %u, total_valid_block_count = %u\n",
					from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()), sbi->user_block_count, sbi->total_valid_block_count );
		...
		if (unlikely(sbi->total_valid_block_count > sbi->user_block_count)) {
			...
			*count -= diff;
			...
			if (!*count) {
				...
				WARN(1,"no reserved block left, current_fsuid = %u, current_fsgid = %u\n",
					from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()));
				return false;
			}
		}
	} else {
		if (unlikely(sbi->total_valid_block_count > (sbi->user_block_count - f2fs_r_blocks_count(sbi)))) {
			...
			*count = 0;
			WARN(1,"no free block left, current_fsuid = %u, current_fsgid = %u, user_block_count = %u, total_valid_block_count = %u\n",
				from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()), sbi->user_block_count, sbi->total_valid_block_count );
			return false;
		}
		...
		sbi->total_valid_block_count += (block_t)(*count);
		if(unlikely(sbi->total_valid_block_count > (sbi->user_block_count - f2fs_r_blocks_count(sbi)))) {
			diff = sbi->total_valid_block_count - (sbi->user_block_count - f2fs_r_blocks_count(sbi));
			*count -= diff;
			sbi->total_valid_block_count = sbi->user_block_count - f2fs_r_blocks_count(sbi);
			if (!*count) {
				spin_unlock(&sbi->stat_lock);
				percpu_counter_sub(&sbi->alloc_valid_block_count, diff);
				WARN(1,"no free block left, current_fsuid = %u, current_fsgid = %u\n",
					from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()));
				return false;
			}
		}	
						...
						struct inode *inode)
{
	...
	if (uid_gte(sbi->s_resuid, current_fsuid()) || gid_gte(sbi->s_resgid, current_fsgid()) ||
		...
		if (unlikely(valid_block_count > sbi->user_block_count)) {
			spin_unlock(&sbi->stat_lock);
			WARN(1,"no reserved blocks left, current_fsuid = %u, current_fsgid = %u\n",
				from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()));
			return false;
		}
	} else {
		if (unlikely(valid_block_count > (sbi->user_block_count - f2fs_r_blocks_count(sbi)))) {
			spin_unlock(&sbi->stat_lock);
			WARN(1,"no free blocks left, current_fsuid = %u, current_fsgid = %u, user_block_count = %u, total_valid_block_count = %u\n",
					from_kuid_munged(&init_user_ns, current_fsuid()), from_kgid_munged(&init_user_ns, current_fsgid()), sbi->user_block_count, sbi->total_valid_block_count );
			return false;
		}
	}
