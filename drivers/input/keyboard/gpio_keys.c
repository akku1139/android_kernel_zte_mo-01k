...
#define MUTE_CODE  113

module_param(mute_state, int, 0644);
/*
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
    ...
    if(MUTE_CODE==button->code){
          mute_state=(state)^ button->active_low;
	   }
	pr_info("report button gpio- keys number %d  state %d \n",button->gpio,state);
	...
	if (gpio_is_valid(button->gpio)) {
		...
		if (button->debounce_interval) {
			...
			if (error < 0)
              ...
              if(MUTE_CODE==button->code){
                     mute_state=(gpio_get_value_cansleep(button->gpio) ? 1 : 0);
	       }
	       pr_info("setup button gpio- keys number %d \n",button->gpio);
		//pr_info("button setup  gpio- keys number %d \n",button->gpio);
