...
static int fts_input_dev_report_b(struct ts_event *event, struct fts_ts_data *data)
{
...
#endif
            data->touchs &= ~BIT(event->au8_finger_id[i]);
            //FTS_DEBUG("[B]P%d UP!", event->au8_finger_id[i]); //zhangjian delete
        }
    }
    ...
    if (unlikely(data->touchs ^ touchs))
    {
        for (i = 0; i < data->pdata->max_touch_number; i++)
        {
            ...
            {
                //FTS_DEBUG("[B]P%d UP!", i);//zhangjian delete
    ...
    {
        //FTS_DEBUG("Points All Up!");//zhangjian delete
        ...
        fts_data && fts_data->client)
    {
        ...
        if (*blank == FB_BLANK_UNBLANK)
        {
            if (fts_data->fb_tpd_suspend_flag) 
            {
                ...
                if (!err) 
                {
                    printk("start fb_tpd_resume_wq failed\n");
                    return err;
                }
            }
        }
...
//zhangjian add
    if (data->ts_pinctrl) {
        ...
        if (err)
            FTS_ERROR("failed to select relase pinctrl state\n");
        }
    }
#if FTS_POWER_SOURCE_CUST_EN
...
#if FTS_GESTURE_EN  
    if ( data->wakeup_gesture_enable ) //zhangjian add for tp wakeup gesture
    {	
