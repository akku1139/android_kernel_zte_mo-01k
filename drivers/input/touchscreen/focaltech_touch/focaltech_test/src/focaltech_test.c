...
static int tpd_test_cmd_show(struct tpd_classdev_t *cdev, char *buf)
{
    ...
    mutex_lock(&fts_input_dev->mutex);
    disable_irq(client->irq);        
    i_len = sprintf(buf, "%d,%d,%d,%d", g_int_tptest_result , g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum, fts_test.rawdata_failed_count);
    printk("tpd %s [func] test resutl:0x%x && rawdata node failed count:0x%x.\n", __func__, g_int_tptest_result , fts_test.rawdata_failed_count);
    printk("tpd %s [func] resutl && failed cout string lenght:0x%x.\n", __func__, i_len);
    
    buffer_length = (fts_test.special_buffer_length + 1) > (PAGE_SIZE - i_len) ? (PAGE_SIZE - i_len - 1) : fts_test.special_buffer_length;
    printk("tpd %s [func] failed node string lenght:0x%x, buffer_length:0x%x.\n", __func__, fts_test.special_buffer_length, buffer_length);
...
static int tpd_test_cmd_store(struct tpd_classdev_t *cdev, const char *buf)
{
    ...
    {       
        ...
        if(NULL == fts_test.special_buffer ) 
        {
    ...
    else if(2 == command)
    {
        ...
        {			
            FTS_TEST_DBG("command:0x%ld, but node not opened.", command);
        }
    } 
    else if(3 == command) 
    {
