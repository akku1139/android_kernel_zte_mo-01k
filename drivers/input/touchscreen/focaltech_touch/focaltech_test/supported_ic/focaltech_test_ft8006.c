...
int FT8006_get_tp_node_data(int type, char * buffer, int length)
{
    ...
    FTS_TEST_INFO("Tx:%d Rx:%d, iLen:%d, buffer length:%d", g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum, iLen, length);
    ...
    for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
    {
           ...
           )
        {
            ...
            if (ERROR_CODE_OK != ReCode || (!bTempResult))
            {
                ...
                printk("tpd short circuit failed\n");
            }
            else
                g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
        }
