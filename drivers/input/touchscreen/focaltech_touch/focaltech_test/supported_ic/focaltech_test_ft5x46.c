...
extern struct fts_test_st fts_test;
/////////////////////////////////////////////////Reg
...
int FT5X46_get_tp_node_data(int type, char * buffer, int length)
{
	...
	FTS_TEST_INFO("Tx:%d Rx:%d, iLen:%d, buffer length:%d", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum, iLen, length);
...
static int save_rawdata_test_result(char * tmp_buffer, int length)
{
	//TPD_TEST_DBG("length:%d", length);
	
	if(NULL == fts_test.special_buffer || (fts_test.special_buffer_length + length) > TEST_RESULT_LENGTH) {
		FTS_TEST_DBG("warning:buffer is null or buffer overflow, return");
		return -1;
	}
    ...
    if (g_ScreenSetParam.isNormalize == Auto_Normalize)
    {
        ...
        for (iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++)
        {
            for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
            {
                ...
                {
                    btmpresult = false;
                    if(NULL != fts_test.temp_buffer)
                    {
                        i_len = snprintf(fts_test.temp_buffer, TEST_TEMP_LENGTH, ",%d,%d", iRow, iCol);
                        save_rawdata_test_result(fts_test.temp_buffer, i_len);
...
static unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer  )
{
	...
	if(0 != (iReadDataLen%WEAK_SHORT_BYTES_PER_TIME)) iReadNum++;
	...
	for(i=1; i<iReadNum; i++)
	{
		...
		{
			focal_msleep(10);
			ReCode = Comm_Base_IIC_IO(NULL, 0, pDataSend + 1+WEAK_SHORT_BYTES_PER_TIME *i, iReadDataLen-WEAK_SHORT_BYTES_PER_TIME *i);
			if(ReCode != ERROR_CODE_OK)
			{	
		...
		{
			focal_msleep(10);
			ReCode = Comm_Base_IIC_IO(NULL, 0, pDataSend + 1+WEAK_SHORT_BYTES_PER_TIME *i, WEAK_SHORT_BYTES_PER_TIME );
