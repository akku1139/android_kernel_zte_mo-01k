...
static int save_rawdata_test_result(struct synaptics_test_st *stp_test, char * tmp_buffer, int length)
{
	//TPD_TEST_DBG("length:%d", length);
	
	if(NULL == stp_test->special_buffer || (stp_test->special_buffer_length + length) > TEST_RESULT_LENGTH) {
...
int save_string_to_buffer(struct synaptics_test_st *stp_test, char * tmp_buffer, int length)
{
	...
	if(NULL == stp_test->result_buffer || (stp_test->result_length + length) > TEST_RESULT_LENGTH) {
		TPD_TEST_DBG("warning:buffer is null or buffer overflow, return");
		return -1;
	}
...
int save_ito_array_data_to_file(struct synaptics_test_st *stp_test, char *title_info, void *data, int length, int size, int i_row, int i_col)
{
	int i_len = 0;
	int i = 0, j = 0;
	int i_value = 0;
	...
	if(NULL == stp_test->temp_buffer || NULL == stp_test->result_buffer) {
...
void show_rawdata(struct synaptics_test_st *stp_test, int *buffer) {
	...
	for(i_row = 0; i_row<stp_test->screen_cfg.i_txNum; i_row++) {
		for(i_col = 0; i_col < stp_test->screen_cfg.i_rxNum; i_col++) {
			printk("%-4d ", buffer[i_row * stp_test->screen_cfg.i_rxNum + i_col]);
		}
		printk("\n");
	}	
}
...
int synaptics_full_rawdata_test(struct synaptics_test_st *stp_test, int times)
{
	...
	array_rawdata = (int *)tpd_malloc(full_raw_report_size);
	if (!array_rawdata) {
	...
	for(i_row = 0; i_row<stp_test->screen_cfg.i_txNum; i_row++) {
		for(i_col = 0; i_col < stp_test->screen_cfg.i_rxNum; i_col++) {
			...
			if(i_value < i_min) {
				...
				TPD_TEST_DBG("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ",  \
	...
	//Save Test Data
	save_ito_array_data_to_file(stp_test, "rawdata test", array_rawdata, full_raw_report_size, sizeof(int), stp_test->screen_cfg.i_txNum, stp_test->screen_cfg.i_rxNum);
	
	if (NULL != array_rawdata) {
