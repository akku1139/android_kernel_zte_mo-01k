...
static char CFG_SSL = '[';  /* Section Symbol --Can be defined according to the special need to change. For example {}  */
static char CFG_SSR = ']';  /*  Section Symbol --Can be defined according to the special need to change. For example {} */
...
ST_INI_FILE_DATA *syna_g_st_ini_file_data = NULL;
...
Note:
*************************************************************/
int syna_ini_get_key_value(char * section, char * key, char * value)
{
	int i = 0;
	int ret = -2;
	for (i = 0; i < g_used_key_num; i++) {
		if (tpd_strncmp(section, syna_g_st_ini_file_data[i].pSectionName,
						syna_g_st_ini_file_data[i].iSectionNameLen) != 0)
			continue;
		//TPD_TEST_DBG("Section Name:%s, Len:%d\n",  g_st_ini_file_data[i].pSectionName, g_st_ini_file_data[i].iSectionNameLen);
		if (tpd_strncmp(key, syna_g_st_ini_file_data[i].pKeyName,  strlen(key)) == 0) {
			//g_st_ini_file_data[i].iKeyNameLen) == 0)
			memcpy(value, syna_g_st_ini_file_data[i].pKeyValue, syna_g_st_ini_file_data[i].iKeyValueLen);
			ret = 0;
			break;
		}
	}
...
Note:
*************************************************************/
static int syna_ini_file_get_line(char *ini_param, char *buffer, int maxlen)
{
	int i=0;
	int j=0;
	int iRetNum=-1;
...
int syna_init_key_data_space(void)
{
	int i = 0;

	g_used_key_num = 0;

	syna_g_st_ini_file_data =NULL;
	if (NULL == syna_g_st_ini_file_data)
		syna_g_st_ini_file_data = tpd_malloc(sizeof(ST_INI_FILE_DATA)*MAX_KEY_NUM);
	if (NULL == syna_g_st_ini_file_data) 	{
	...
	for (i = 0; i < MAX_KEY_NUM; i++) {
		memset(syna_g_st_ini_file_data[i].pSectionName, 0, MAX_KEY_NAME_LEN);
		memset(syna_g_st_ini_file_data[i].pKeyName, 0, MAX_KEY_NAME_LEN);
		memset(syna_g_st_ini_file_data[i].pKeyValue, 0, MAX_KEY_VALUE_LEN);
		syna_g_st_ini_file_data[i].iSectionNameLen = 0;
		syna_g_st_ini_file_data[i].iKeyNameLen = 0;
		syna_g_st_ini_file_data[i].iKeyValueLen = 0;
	}
...
{
	if (NULL != syna_g_st_ini_file_data) {
		tpd_free(syna_g_st_ini_file_data);
		syna_g_st_ini_file_data = NULL;
	}
...
int debug_print_key_data(void)
{
	...
	for (i = 0; i < MAX_KEY_NUM; i++)
	{
		...
		TPD_TEST_DBG("pSectionName_%d:%s, pKeyName_%d:%s\n,pKeyValue_%d:%s",
					 i, syna_g_st_ini_file_data[i].pSectionName,
					 i, syna_g_st_ini_file_data[i].pKeyName,
					 i, syna_g_st_ini_file_data[i].pKeyValue
					);
...
int syna_ini_get_key_data(char *ini_param)
{
	...
	//malloc ini section/key/value data space.
	retval = syna_init_key_data_space();
	if (retval < 0) {
	...
	while (1) {
		...
		line_len = syna_ini_file_get_line(ini_param + offset, cfg_onebuf, MAX_CFG_BUF);
		if (line_len < -1)
			goto cfg_scts_end;
		if (line_len < 0)
			break;/* file end */
		...
		if (cfg_onebuf[0] == CFG_SSL) {
			syna_g_st_ini_file_data[g_used_key_num].iSectionNameLen = line_len-2;
			if (MAX_KEY_NAME_LEN < syna_g_st_ini_file_data[g_used_key_num].iSectionNameLen) {
			...
			strcpy((char *)tmpSectionName, cfg_onebuf + 1);
			//TPD_TEST_DBG("Section Name:%s, Len:%d\n",  tmpSectionName, n-2);
		...
		strcpy( syna_g_st_ini_file_data[g_used_key_num].pSectionName, tmpSectionName);
		syna_g_st_ini_file_data[g_used_key_num].iSectionNameLen = strlen(tmpSectionName);
		...
		syna_g_st_ini_file_data[g_used_key_num].iKeyNameLen = iEqualSign;
		if (MAX_KEY_NAME_LEN < syna_g_st_ini_file_data[g_used_key_num].iKeyNameLen) {
			retval = CFG_ERR_OUT_OF_LEN;
			TPD_TEST_DBG("MAX_KEY_NAME_LEN: CFG_ERR_OUT_OF_LEN\n");
			goto cfg_scts_end;
		}
		memcpy(syna_g_st_ini_file_data[g_used_key_num].pKeyName,
			   cfg_onebuf, syna_g_st_ini_file_data[g_used_key_num].iKeyNameLen);

		/* After equal sign is assigned to the key value*/
		syna_g_st_ini_file_data[g_used_key_num].iKeyValueLen = line_len-iEqualSign-1;
		if (MAX_KEY_VALUE_LEN < syna_g_st_ini_file_data[g_used_key_num].iKeyValueLen) {
			retval = CFG_ERR_OUT_OF_LEN;
			TPD_TEST_DBG("MAX_KEY_VALUE_LEN: CFG_ERR_OUT_OF_LEN\n");
			goto cfg_scts_end;
		}
		memcpy(syna_g_st_ini_file_data[g_used_key_num].pKeyValue,
			   cfg_onebuf+ iEqualSign+1, syna_g_st_ini_file_data[g_used_key_num].iKeyValueLen);
...
int get_key_value_string(char *section, char *ItemName, char *defaultvalue, char *returnValue)
{
	...
	if (syna_ini_get_key_value(section, ItemName, value) < 0) {
