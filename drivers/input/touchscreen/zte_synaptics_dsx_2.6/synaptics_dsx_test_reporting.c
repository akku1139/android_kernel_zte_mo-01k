...
#ifdef F54_SHOW_MAX_MIN
	pr_notice("%s : image part 1 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif
...
#ifdef F54_SHOW_MAX_MIN
	pr_notice("%s : image part 2 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif
...
#ifdef F54_SHOW_MAX_MIN
	pr_notice("%s : data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("synaptics : amp open test setp1 2016121401 dur lsb:0x%x msb:0x%x rst:0x%x\n", control.reg_99->integration_duration_lsb, control.reg_99->integration_duration_msb, 
	...
	printk("synaptics setp 1 dst size:0x%x, scr size0x%x\n",  tx_num * rx_num * 2, f54->report_size);
	
	secure_memcpy(p_report_data_8, tx_num * rx_num * 2,
		f54->report_data, f54->report_size, tx_num * rx_num * 2);//f54->report_size);

	printk("synaptics setp 1:0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", p_report_data_8[0], p_report_data_8[1], p_report_data_8[2], p_report_data_8[3], 
		p_report_data_8[4], p_report_data_8[5]);
	printk("synaptics setp 1 f54:0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", f54->report_data[0], f54->report_data[1], f54->report_data[2], f54->report_data[3], 
		f54->report_data[4], f54->report_data[5]);
	...
	control.reg_99->reset_duration = original_data_f54_ctrl99[2];
	printk("synaptics : amp open test setp2 dur lsb:0x%x msb:0x%x rst:0x%x\n", control.reg_99->integration_duration_lsb, control.reg_99->integration_duration_msb, 
		control.reg_99->reset_duration);
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_99->address,
			control.reg_99->data,
	...
	printk("synaptics setp 2:0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", p_report_data_8[0], p_report_data_8[1], p_report_data_8[2], p_report_data_8[3], 
		p_report_data_8[4], p_report_data_8[5]);
	printk("synaptics setp 2 f54:0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", f54->report_data[0], f54->report_data[1], f54->report_data[2], f54->report_data[3], 
		f54->report_data[4], f54->report_data[5]);
	// normalize the rt92 image with 16-bit
	k = 0;
	for (i = 0; i < tx_num; i++) {
	...
	control.reg_99->reset_duration = original_data_f54_ctrl99[2];
	printk("synaptics : amp open test restore dur lsb:0x%x msb:0x%x rst:0x%x\n", control.reg_99->integration_duration_lsb, control.reg_99->integration_duration_msb, 
		control.reg_99->reset_duration);
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_99->address,
			control.reg_99->data,
...
#ifdef F54_SHOW_MAX_MIN
	pr_notice("%s : ph.1 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif
...
#ifdef F54_SHOW_MAX_MIN
	pr_notice("%s : ph.2 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < button_count; i++) {
		long value = p_rt92_image_1[(tx_num-1)*rx_num + i] - p_rt92_image_2[(tx_num-1)*rx_num + i];
...
//#endif
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s: syna_dbg ctrl_182 addr=0x%-4x, data_0=%x, data_1=%x, data_2=%x, data_3=%x\n",
				__func__, control.reg_182->address, original_f54_ctrl182.data[0], original_f54_ctrl182.data[1], original_f54_ctrl182.data[2], original_f54_ctrl182.data[3]);
#if 0
	/* step 1 */
	...
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to do force update before changing integration duration\n",
				__func__);
		retval = -EIO;
		goto exit;
	}
...
static int synaptics_rmi4_test_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	init_tpd_test_attributes(&tpd_fw_cdev);
#endif
...
int tpd_synaptics_get_full_rawdata(int *rawdata, int length)
{
	...
	printk("synaptics %s func in\n", __func__);

	if(NULL == rawdata || length < tx_num * rx_num)
		return -1;
	...
	secure_memcpy(pchar_raw_data_buffer, full_raw_report_size,
		f54->report_data, f54->report_size, full_raw_report_size);    // f54->report_size);
	...
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < rx_num; jj++) {
			...
			printk("#%-4d %-4d ", *report_data_16,  rawdata[count] );
			report_data_16++;
			count++;
		}
		printk("\n");
	}
	printk("print td43xx full raw data\n");
	report_data_16 = (short *)pchar_raw_data_buffer;
	for (ii = 0; ii < tx_num; ii++) {
		for (jj = 0; jj < (rx_num - 1); jj++) {
			printk("#%-4d %-4d ", *report_data_16,  rawdata[ii * rx_num + jj] );
			report_data_16++;
...
//		kfree(g_tddi_amp_open_data_output);
	...
	save_ito_array_data_to_file(stp_test, "amp open data 1", p_rt92_image_1, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "amp open data 2", p_rt92_image_2, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "amp open delta data", p_rt92_delta_image, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	/* step 4 */
	/* phase 1, the delta value form the above two rt92 images */
	/* should be within the phase 1 test limit*/
...
//			if ((p_rt92_delta_image[i * rx_num + j] < AMP_OPEN_TEST_LIMIT_PHASE1_LOWER) ||
//				(p_rt92_delta_image[i * rx_num + j] > AMP_OPEN_TEST_LIMIT_PHASE1_UPPER)) {
			if ((p_rt92_delta_image[i * rx_num + j] < stp_test->basic_threshold.amp_open_test_limit_phase1_lower) ||
				(p_rt92_delta_image[i * rx_num + j] > stp_test->basic_threshold.amp_open_test_limit_phase1_upper)) {
...
//#endif
	...
	save_ito_array_data_to_file(stp_test, "amp open test phase1 limit: ", p_rt92_image_1, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	...
	save_ito_array_data_to_file(stp_test, "amp open test ratio data: ", p_rt92_delta_image, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
...
//			if ((p_rt92_delta_image[i * rx_num + j] < AMP_OPEN_TEST_LIMIT_PHASE2_LOWER) ||
//				(p_rt92_delta_image[i * rx_num + j] > AMP_OPEN_TEST_LIMIT_PHASE2_UPPER)) {
			if ((p_rt92_delta_image[i * rx_num + j] < stp_test->basic_threshold.amp_open_test_limit_phase2_lower) ||
				(p_rt92_delta_image[i * rx_num + j] > stp_test->basic_threshold.amp_open_test_limit_phase2_upper)) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d at phase 2 (limit = %d, %d)\n",
						__func__, i, j, p_rt92_delta_image[i*rx_num + j],
						AMP_OPEN_TEST_LIMIT_PHASE2_LOWER, AMP_OPEN_TEST_LIMIT_PHASE2_UPPER);
...
//	pr_notice("%s : ph.2 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
//#endif
	save_ito_array_data_to_file(stp_test, "amp open test phase2 limit: ", p_rt92_image_2, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
...
//#endif
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 1 image", tddi_rt95_part_one, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
...
//			if (tddi_rt95_part_one[i*rx_num + j] > EE_SHORT_TEST_LIMIT_PART1) {
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 1 limit", tddi_rt95_part_one, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
...
//#endif
	...
	save_ito_array_data_to_file(stp_test, "eeshort test part 2 image", tddi_rt95_part_two, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
	
	// calculate the ratio
	tddi_ratio_calculation(tddi_rt95_part_two);
	save_ito_array_data_to_file(stp_test, "eeshort test calculate the ratio", tddi_rt95_part_two, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
...
//#endif
//			if (tddi_rt95_part_two[i*rx_num + j] < EE_SHORT_TEST_LIMIT_PART2) {
			if (tddi_rt95_part_two[i*rx_num + j] < stp_test->basic_threshold.ee_short_test_limit_part2) {
				dev_err(f54->rmi4_data->pdev->dev.parent,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d in part 2 image (limit = %d)\n",
						__func__, i, j, tddi_rt95_part_two[i*rx_num + j], EE_SHORT_TEST_LIMIT_PART2);
...
//	pr_notice("%s : image part 2 data range (max, min) = (%-4d, %-4d)\n", __func__, max, min);
//#endif
	save_ito_array_data_to_file(stp_test, "eeshort test part 2 limit", tddi_rt95_part_two, tx_num * rx_num, sizeof(unsigned short), tx_num, rx_num);
