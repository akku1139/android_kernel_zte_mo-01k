...
//for glove_mode
static DEVICE_ATTR(state, (S_IRUGO | S_IWUSR | S_IWGRP),  synaptics_rmi4_glove_mode_show,  synaptics_rmi4_glove_mode_store); 

static struct attribute *synaptics_glove_mode_attributes[] = {	
	&dev_attr_state.attr,		
	NULL
};
static struct attribute_group synaptics_glove_mode_attribute_group = {	
	.attrs = synaptics_glove_mode_attributes
};
//end
...
//add by wkh for glove mode begin
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
			/*rmi4_data->enable_glove_mode*/enable_glove_mode_state);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(input == 1)
	{
        ...
        retval = synaptics_rmi4_reg_write(rmi4_data,
			0x001b,
			&state_control,
			sizeof(state_control));	
		if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to write 0x001b to enter glove mode\n",
				__func__);
		...
		if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to write 0x0307 glove mode\n",
				__func__);
		//return;
		}*/
	}
	...
	/*rmi4_data->enable_glove_mode*/enable_glove_mode_state = input;
	return count;
}
//end
#ifdef USE_DATA_SERVER
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->power_gpio >= 0) {
		gpio_direction_output(bdata->power_gpio, bdata->power_on_state);
		...
		bool enable)
{
	...
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
      static bool TP_power_on=false;
	if ((!enable) &&TP_power_on){
		retval = 0;
...
disable_pwr_reg:
	if (rmi4_data->pwr_reg){
		regulator_disable(rmi4_data->pwr_reg);
             printk(" syna disable regulators pwr_reg \n");
		}
disable_bus_reg:
	if (rmi4_data->bus_reg){
		regulator_disable(rmi4_data->bus_reg);
             printk(" syna disable regulators bus_reg\n");
		}
exit:
	TP_power_on =false;
	return retval;
...
//zz	
//zhangjian add
extern int synaptics_get_tscinfo(char *p_chip_id,char *p_vendor_id,int *p_fw_ver );
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	int fwver_in_chip = 0;
	char vendorid_in_chip = 0;
	char chipid_in_chip = 0;
	
	synaptics_get_tscinfo(&chipid_in_chip,&vendorid_in_chip,&fwver_in_chip);
...
static int tpd_get_wakegesture(struct tpd_classdev_t *cdev)
{
    ...
    printk("%s wakeup_gesture_enable val is:%d.\n", __func__, ts->enable_wakeup_gesture);
    cdev->b_gesture_enable = ts->enable_wakeup_gesture;
    return 0;
}
...
static int synaptics_TP_ESD_check(struct tpd_classdev_t *cdev)
{
    ...
    if (rmi4_data->stay_awake)
		return 0;
    retval = synaptics_rmi4_reg_read(rmi4_data,0x0003,esd_read_buff,2);
    flash_status = esd_read_buff[0];
    device_status = esd_read_buff[1];
    pr_notice("%s: flash_status is %d,device_status is %d\n", __func__,flash_status,device_status);
    if ((flash_status != 0) ||(device_status !=0)){   
        dev_err(rmi4_data->pdev->dev.parent,
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	tpd_register_fw_class(rmi4_data);//zhangjian add
	// add by wkh for create tp_wake_gesture
		...
		if (0 != retval)	 
		{		 
			printk("syna--ERROR: sysfs_create_gesture_group() failed.\n");		  
    ...
    //end
    //	create glove mode node begin
     	synaptics_tp_glove_mode_kobj = kobject_create_and_add("tp_glove_mode", NULL);	 
		if (!synaptics_tp_glove_mode_kobj)	
		{		 
			retval = -EINVAL;		 
			printk("syna--ERROR Unable to create shtsc_glove_mode_kobj\n");	 
		}	 
		retval = sysfs_create_group(synaptics_tp_glove_mode_kobj, &synaptics_glove_mode_attribute_group);	 
		if (0 != retval)	 
		{		 
			printk("syna--ERROR: sysfs_create_glove_group() failed.\n");		  
			//sysfs_remove_group(&client->dev.kobj, &gt9xx_wake_gesture_attribute_group);		 
			kobject_put(synaptics_tp_glove_mode_kobj);		
			//return -EIO;	
		}	  
		else	 
		{		 
			printk("GTP:sysfs_create_glove_group() succeeded.\n");    
		}
    //end
//zz add by wkh for touch info
...
if(synaptics_tp_glove_mode_kobj)
	{
		sysfs_remove_group(synaptics_tp_glove_mode_kobj, &synaptics_glove_mode_attribute_group);	
		kobject_put(synaptics_tp_glove_mode_kobj);
	}
//end
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && rmi4_data && rmi4_data->pdev) {
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval;//zhangjian add for power off
      const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	if (rmi4_data->stay_awake)
		return 0;
...
//zhangjian add for power off
	retval = synaptics_rmi4_enable_reg(rmi4_data, false);
	if (retval < 0) {
		printk("Failed to disable regulators\n");
	}
	if (bdata->reset_gpio >= 0) {
...
//#endif
	...
	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
	printk("syna,synaptics_rmi4_resume power on\n");
	if (retval < 0) {
		printk("Failed to enable regulators\n");
	}
	if (bdata->reset_gpio >= 0) {
		gpio_direction_output(bdata->reset_gpio,0);
		msleep(bdata->reset_active_ms);
		gpio_direction_output(bdata->reset_gpio, 1);
		msleep(bdata->reset_delay_ms);
		
	}
    /* add by zsw wkh begin */
	if(enable_glove_mode_state)
	{
	    unsigned char state_control;
		state_control = 0x25;
        retval = synaptics_rmi4_reg_write(rmi4_data,
...
exit:
	...
	//printk("syna,synaptics_rmi4_resume,enter here\n");
       ...
       if (LCM_esd_recovery)
       {
           LCM_esd_recovery = false;
           pr_notice("%s: TP ESD spontaneous reset detected\n", __func__);
           retval = synaptics_rmi4_reinit_device(rmi4_data);
           if (retval < 0) {
               dev_err(rmi4_data->pdev->dev.parent, "%s: Failed to reinit device\n", __func__);       
           }
       }
	   //add end
	if (rmi4_data->enable_sleep_mode)
