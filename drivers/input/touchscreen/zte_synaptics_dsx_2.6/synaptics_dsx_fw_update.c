...
#ifdef CONFIG_FIRMWARE_CONFIG
#include "synaptics_TP_config.h"
#endif
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
          #ifndef CONFIG_FIRMWARE_CONFIG //zhangjian modify
          retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
          FW_IMAGE_NAME, sizeof(FW_IMAGE_NAME),
          sizeof(FW_IMAGE_NAME));
          #else
          switch (fwu->config_id[1])
          {
              printk("%s: VENDOR_ID is %x\n",__func__,fwu->config_id[1]);
              #if (SYNAPTICS_GET_VENDOR_ID_NUM   >= 1)              
              case SYNAPTICS_VENDOR_1_ID:
                  printk("%s: SYNAPTICS image_name is %s\n",__func__,SYNAPTICS_UPGRADE_FW_IMG);	    
                  retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                  SYNAPTICS_UPGRADE_FW_IMG, sizeof(SYNAPTICS_UPGRADE_FW_IMG),
                  sizeof(SYNAPTICS_UPGRADE_FW_IMG));
                  break;
              #endif 
              #if (SYNAPTICS_GET_VENDOR_ID_NUM   >= 2)  
              case SYNAPTICS_VENDOR_2_ID:
                  printk("%s: SYNAPTICS image_name is %s\n",__func__,SYNAPTICS_UPGRADE_FW2_IMG); 		
                  retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                  SYNAPTICS_UPGRADE_FW2_IMG, sizeof(SYNAPTICS_UPGRADE_FW2_IMG),
                  sizeof(SYNAPTICS_UPGRADE_FW2_IMG));
                  break;   
              #endif
              #if (SYNAPTICS_GET_VENDOR_ID_NUM   >= 3)  
              case SYNAPTICS_VENDOR_3_ID:
                  printk("%s: SYNAPTICS image_name is %s\n",__func__,SYNAPTICS_UPGRADE_FW3_IMG);			  	
                  retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                  SYNAPTICS_UPGRADE_FW3_IMG, sizeof(SYNAPTICS_UPGRADE_FW3_IMG),
                  sizeof(SYNAPTICS_UPGRADE_FW3_IMG));
                  break;
              #endif  
              default:
                  dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to copy image file name\n",__func__);
                  retval =-1;
                  break;		
          }
          #endif
...
int getConfigId(char *buf)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,"%s: config_id[2] is %d     config_id[3] is %d  \n",__func__,config_id[2],config_id[3] );
	 ...
	 else if(config_id[0] == 86 && config_id[1] == 70)
        {
	    return snprintf(buf, 100, "TD4100+BOE(%d.%d.%d.%d)\n",config_id[0], config_id[1], config_id[2], config_id[3]);
        }	
...
//end
//zhangjian add
int synaptics_get_tscinfo(char *p_chip_id,char *p_vendor_id,int *p_fw_ver )
{
    struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
    unsigned char config_id[32];
    int retval = 0;
    ...
    printk("%s: config_id[0] is %d     config_id[1] is %d  \n",__func__,config_id[0],config_id[1] );
    printk("%s: config_id[2] is %d     config_id[3] is %d  \n",__func__,config_id[2],config_id[3] );
    *p_chip_id = config_id[0];
    *p_vendor_id=config_id[1];
    *p_fw_ver= config_id[2] <<8 |config_id[3];     
    return 0;		
}
//add end
static ssize_t fwu_sysfs_write_guest_code_store(struct device *dev,
