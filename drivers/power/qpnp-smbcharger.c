...
#ifdef CONFIG_QPNP_SMBCHARGER_SHUTDOWN_NODE
#include <linux/completion.h> //lixinghe 20170309. shutdown node.
...
struct smbchg_chip {
	...
	//lixinghe 20170309. shutdown node. start 
#ifdef CONFIG_QPNP_SMBCHARGER_SHUTDOWN_NODE
	struct work_struct		driver_shutdown_work;	
	atomic_t  driver_shutdown_start_flag;
	struct completion driver_shutdown_completion;
#endif
	//lixinghe 20170309. shutdown node. end
#ifdef CONFIG_QPNP_SMBCHARGER_BATTERY_CHIP_CODE
	int battery_chip_code;
#endif
...
#ifdef CONFIG_QPNP_SMBCHARGER_ZTE_FEATURE_PARALLEL_CHARGE
...
static int zte_shutdown_disabled(const char *val, struct kernel_param *kp)
{
	int ret;
...
module_param_call(shutdown_disabled, zte_shutdown_disabled, param_get_uint,
					&shutdown_disabled, 0660);

static struct smbchg_chip *the_chip;
static int charging_disabled = 0;
static int smbchg_charging_en(struct smbchg_chip *chip, bool en);
static int zte_charging_disabled(const char *val, struct kernel_param *kp)
{
	int ret;
...
module_param_call(charging_disabled, zte_charging_disabled, param_get_uint,
...
static int zte_set_usb_offline(const char *val, struct kernel_param *kp)
{
	int offline;
       ...
       pr_err("zte_set_usb_offline,start\n");
	...
	pr_err("zte_set_usb_offline,end\n");
	return 0;
} 
...
static void smbchg_parallel_usb_en_work(struct work_struct *work)
{
	...
	/** ZSW_DingXuehua, limit charge current when battery warm or cool via disable parallel charger */
	if(chip->batt_warm || chip->batt_cool){
		pr_info("battery temp warm or cool\n");
	}
	//if (smbchg_is_parallel_usb_ok(chip, &total_current_ma)) {
	if ((!chip->batt_warm) && (!chip->batt_cool) && smbchg_is_parallel_usb_ok(chip, &total_current_ma)){
	/** ZSW_DingXuehua, limit charge current when battery warm or cool via disable parallel charger */
...
static int smbchg_config_chg_battery_type(struct smbchg_chip *chip)
{
	...
	rc = of_property_read_u32(profile_node, "qcom,batt-chip-code",
						&chip->battery_chip_code);
	if (rc) {
		pr_err("couldn't find read qcom,batt-chip-code.\n");
		chip->battery_chip_code = -1;
	}
#endif
	/* change vfloat */
...
#endif	
#ifdef CONFIG_QPNP_SMBCHARGER_BATTERY_CHIP_CODE
	POWER_SUPPLY_PROP_BATTERY_CHIP_CODE,
#endif
};
...
#endif
#ifdef CONFIG_QPNP_SMBCHARGER_BATTERY_CHIP_CODE
	case POWER_SUPPLY_PROP_BATTERY_CHIP_CODE:
		val->intval = chip->battery_chip_code;
		break;
#endif
	default:
		return -EINVAL;
...
static irqreturn_t usbin_ov_handler(int irq, void *_chip)
{
	...
	if (reg & USBIN_OV_BIT) {
		...
		if (chip->usb_psy) {
			...
			/** ZSW modify, dont show charging icon in status bar, except for offcharging mode */
			if(!charger_boot && probe_done_flag)
...
static void smbchg_shutdown(struct spmi_device *spmi)
{
	...
	if(start_flag == true)
	{
		wait_for_completion(&chip->driver_shutdown_completion);
		return;
	}
	else
