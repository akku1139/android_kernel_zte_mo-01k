...
int zte_warm_reset = 0;
static int __init zte_warm_reset_setup(char *str) 
{
	if(get_option(&str, &zte_warm_reset))
	{
	    printk("zte_warm_reset = %d\n", zte_warm_reset);
	    return 0;
	}
	else
	{
	    printk("zte_warm_reset get failed \n");
	    return -EINVAL;
	}
}
early_param("zte_warm_reset", zte_warm_reset_setup);
...
int qpnp_pon_is_warm_reset(void)
{
    ...
    dev_emerg(&pon->spmi->dev, "PMIC@SID%d Warm-reset reason1: 0x%x\n", pon->spmi->sid, pon->warm_reset_reason1);
    dev_emerg(&pon->spmi->dev, "PMIC@SID%d Warm-reset reason2: 0x%x\n", pon->spmi->sid, pon->warm_reset_reason2);
...
static int qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	for_each_available_child_of_node(pon->spmi->dev.of_node, pp) {
		...
		if (cfg->support_reset) {
            ...
            if ((PON_KPDPWR == cfg->pon_type) && zte_warm_reset)
            {
                cfg->s2_type = 1;
            }
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
    ...
    dev_emerg(&pon->spmi->dev, "PMIC@SID%d Power-on reason: 0x%x\n", pon->spmi->sid, pon_sts); 
    ...
    dev_emerg(&pon->spmi->dev, "PMIC@SID%d Power-off reason: 0x%x\n", pon->spmi->sid, poff_sts); 
