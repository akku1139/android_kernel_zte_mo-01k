...
int msm_dss_vreg_power_off(void)
{
	int ret = 0;
	...
	pdata = &G_ctrl_pdata->panel_data;
	ret = mdss_dsi_panel_reset(pdata, 0);
	if (ret) {
		pr_warn("%s: Panel reset failed. rc=%d\n", __func__, ret);
		ret = 0;
	}
	if (mdss_dsi_pinctrl_set_state(G_ctrl_pdata, false))
		pr_debug("reset disable: pinctrl not enabled\n");
	
	pr_err("[MSM_LCD]:vreg_power_off\n");	
	ret = msm_dss_enable_vreg(
		G_ctrl_pdata->panel_power_data.vreg_config,
		G_ctrl_pdata->panel_power_data.num_vreg, 0);	
	return ret;
...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	pr_err("[MSM_LCD]%s:power off\n", __func__);//yangchaofeng add log info
	#endif
	#ifndef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
	ret = msm_dss_enable_vreg(
