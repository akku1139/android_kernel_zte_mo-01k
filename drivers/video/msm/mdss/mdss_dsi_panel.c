...
extern  int TP_ESD_check(void);//zhangjian add for TP esd check
//lixuetao add for read panel info
...
static int blmax_lcm_10bit=0;         //modify by yujianhua start for V8 OTM  lcm bl level 0x3ff
...
{		
	...
	if (!strcmp(panel_name, "zteLEAD(hx8399c+auo)_1080*1920-5.2Inch"))
	{
		blmax_lcm_12bit=0x1;
		lcm_reset_high_insleep=0x1;
	}
	
	if (!strcmp(panel_name, "zte(otm1911a)_1080*1920-5.2Inch"))
	{
		blmax_lcm_10bit=1;
		lcm_reset_high_insleep=1;
	}
	
	if (!strcmp(panel_name, "zteSkyworth(ili9881c)_720*1280-5.0Inch"))
	{
		lcm_reset_high_insleep=0x0;
	}

    if (!strcmp(panel_name, "zteBOE(td4300)_1080*1920-5.5Inch")||!strcmp(panel_name, "zteTianMa(ft8716)_1080*1920-5.5Inch")||!strcmp(panel_name, "zteTianMa(td4310)_1080*1920-5.5Inch"))
	{
...
extern struct zte_lcd_info g_zte_lcd_info;

static char BL_page_23_on[3] = {0x50, 0x5a ,0x23};
static char BL_0x94_on[2] = {0x94, 0x2c};
static char BL_0x90_on[3] = {0x90, 0x00,0x00};

static struct dsi_cmd_desc display_on_CABC_backlight_cmds[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(BL_page_23_on)}, BL_page_23_on},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(BL_0x94_on)}, BL_0x94_on },
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 2, sizeof(BL_0x90_on)}, BL_0x90_on },
};
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	{
		CABC_0x53[1]=0x24;//close dimming
		BL_0x94_on[1]=0x24;
	}
	else if(level==0)
	{
	...
	{
		if(g_zte_lcd_info.is_close_dimming)
		{
			CABC_0x53[1]=0x24;//close dimming
			BL_0x94_on[1]=0x24;
		}
	...
	if(pinfo->bl_max >256)
	{
		BL_0x90_on[1] = (unsigned char)((level >> 4) & 0x0FF);
		BL_0x90_on[2] = (unsigned char)(level & 0x00F);

		printk("[MSM_LCD]%s:level=%d high=0x%x low=0x%x\n", __func__, level,BL_0x90_on[1],BL_0x90_on[2]);

		memset(&cmdreq, 0, sizeof(cmdreq));
		cmdreq.cmds = &display_on_CABC_backlight_cmds[0];
		cmdreq.cmds_cnt = 3;
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
	...
	{
		printk("[MSM_LCD]%s:level=%d\n", __func__, level);

		led_pwm1[1] = (unsigned char)level;

		memset(&cmdreq, 0, sizeof(cmdreq));
		cmdreq.cmds = &backlight_cmd[0];
		cmdreq.cmds_cnt = 2;
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
	...
	else if(blmax_lcm_10bit==1)//only for v8 otm1911a lcm
	{
		if ( (level <= 40) && (level > 0))    
		{
			level = 16;
		...
		//printk("%s:MAXLCM10bit level=%d,cabc=%d,minb=%d\n", __func__, level,lcm_cabc_on,lcm_is_minbright);
	}
	else
	{
		#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
...
//add by yujianhua for cabc exit sleep end
	...
	if(blmax_lcm_12bit==0x1 || blmax_lcm_10bit==0x1)
	{
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
	if (ctrl_pdata->panel_reset_early) {
	    mutex_lock(&ctrl_pdata->panel_reset_lock);
	    printk("mdss_dsi_panel_reset: begin enable=%d.\n", enable);
	    if (ctrl_pdata->last_panel_reset_status == enable) {
	       printk("mdss_dsi_panel_reset: already enable=%d.\n", enable);
		mutex_unlock(&ctrl_pdata->panel_reset_lock);
	       return 0;
	    }
		//mutex_lock(&ctrl_pdata->panel_reset_lock);
	    ctrl_pdata->last_panel_reset_status = enable;
	}
	#endif
	...
	#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
	pr_err("[MSM_LCD]%s: enable = %d\n", __func__,enable);//yangchaofeng add log info
	#endif
	pr_debug("%s: enable = %d\n", __func__, enable);
	...
	if (enable) {
		...
		else
		{
			...
			{
				printk("%s: TP enter WG state, LCM reset high\n", __func__);
				gpio_set_value((ctrl_pdata->rst_gpio), 1);
			}
			else
			{
				printk("%s: TP exit WG state, LCM reset low\n", __func__);
				gpio_set_value((ctrl_pdata->rst_gpio), 0);
			}
		}
		//modify by yujianhua start for F10 lcm rst high in sleep mode
...
exit:
	...
	if (ctrl_pdata->panel_reset_early) {
	    mutex_unlock(&ctrl_pdata->panel_reset_lock);
		printk("mdss_dsi_panel_reset: end.\n");
	}
	#endif
	return rc;
}
							...
							u32 bl_level)
{
	...
	static u32 last_bl=0;//yangchaofeng add
...
//modify by yujianhua for lcm level print end
	...
	/*ZSW_MODIFY begin,modify for lcd common function,yangchaofeng,20170309*/
	#if defined(CONFIG_ZTE_LCD_COMMON_FUNCTION) && defined(ZTE_LCD_COVERT_BACKLEVEL)
		bl_level = zte_covert_backlevel_function(bl_level,ctrl_pdata->panel_data.panel_info.bl_max);
	#else
		printk("mdss_dsi_panel_bl_ctrl, bl_level=%d\n", bl_level);
	#endif
	/*ZSW_MODIFY end,modify for lcd common function,yangchaofeng,20170309*/
  ...
  //add by yujianhua for a new backlight ctrl end
	...
	last_bl=bl_level;//yangchaofeng adc
}
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
	pr_err("[MSM_LCD]%s: ctrl->on_cmds cmd_cnt=%d\n",__func__,ctrl->on_cmds.cmd_cnt);//yangchaofeng add log info
	#endif
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
	pr_err("[MSM_LCD]%s: ctrl->off_cmds  cmd_cnt=%d\n",__func__,ctrl->off_cmds.cmd_cnt);//yangchaofeng add log info
	#endif
...
{
	if (!mdss_dsi_cmp_panel_reg_v2(ctrl_pdata) || TP_ESD_check()){ //zhangjian add for TP esd check add TP_ESD_check()
	...
	struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (!pinfo->esd_check_enabled)
		...
		if (5 != zte_get_boot_mode_func())
		{
			pinfo->esd_check_enabled = false;
			printk("[MSM_LCD]%s:g_zte_boot_mode is %d\n",__FUNCTION__,zte_get_boot_mode_func());
			return;
		}
	#endif
...
#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
extern void  zte_lcd_common_func(struct mdss_dsi_ctrl_pdata *ctrl_pdata,struct device_node *node);//yagnchaofeng add for lcd common function
#endif
#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
struct mutex *panel_reset_lock = NULL;
#endif
	...
	int ndx)
{
	...
	#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
	mutex_init(&ctrl_pdata->panel_reset_lock);
	panel_reset_lock = &ctrl_pdata->panel_reset_lock;
	ctrl_pdata->panel_reset_early = of_property_read_bool(node,
		"zte,panel-reset-early-enabled");
	ctrl_pdata->last_panel_reset_status = 0;
	#endif
	power_off_remove_to_TP = of_property_read_bool(node,
		"zte,power-off-remove-to-tp");
	...
	#ifdef CONFIG_ZTE_LCD_COMMON_FUNCTION
	zte_lcd_common_func(ctrl_pdata,node);//yagnchaofeng add for lcd common function
	#else
	//lixuetao add for read panel info
