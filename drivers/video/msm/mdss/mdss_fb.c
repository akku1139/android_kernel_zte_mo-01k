...
extern int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable);
static void __mdss_dsi_panel_reset_work(struct work_struct *work) {
    struct delayed_work *dw = to_delayed_work(work);
	struct msm_fb_data_type *mfd = container_of(dw, struct msm_fb_data_type,
		panel_reset_ansy_work);
	...
	mdss_dsi_panel_reset(pdata, mfd->panel_enable);
}
#endif
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
	mfd->panel_enable = 0;
	INIT_DELAYED_WORK(&mfd->panel_reset_ansy_work, __mdss_dsi_panel_reset_work);
	#endif
...
#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
extern struct mutex *panel_reset_lock;
static int mdss_fb_panel_pm_resume_early (struct device *dev) {
    struct mdss_panel_data *pdata = NULL;
    struct msm_fb_data_type *mfd = NULL;
	...
	mfd->panel_enable = 1;
	if ((ctrl_pdata->ndx == 0) && (ctrl_pdata->fb_node == 0) 
	    && (panel_reset_lock == &ctrl_pdata->panel_reset_lock)
	    && ctrl_pdata->panel_reset_early) {
	    // start panel  reset on work
	    schedule_delayed_work(&mfd->panel_reset_ansy_work,
		    msecs_to_jiffies(1));
	}
    return 0;
}

static int mdss_fb_panel_pm_suspend_late (struct device *dev) {
	struct mdss_panel_data *pdata = NULL;
    struct msm_fb_data_type *mfd = NULL;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
...
static const struct dev_pm_ops mdss_fb_pm_ops = {
	...
	#ifdef CONFIG_ZTE_LCD_RESET_EARLY_RESUME
	.resume_early = mdss_fb_panel_pm_resume_early,
	.suspend_late = mdss_fb_panel_pm_suspend_late,
	#endif
};
...
void mdss_fb_report_panel_dead(struct msm_fb_data_type *mfd)
{
	...
	LCM_esd_recovery = true; //zhangjian add for synaptics TP
